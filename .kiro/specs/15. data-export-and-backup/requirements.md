...
1. WHEN creating backups THEN the system SHALL use zip-then-encrypt: first package NDJSON per table, manifest.json, checksums.json into a ZIP; then encrypt the ZIP payload with AES-GCM-256 using the user passphrase to produce a single `.cbk` file (encrypted envelope).
   ...
   2. WHEN generating backup files THEN the system SHALL use a portable envelope header placed before the ciphertext: 8-byte magic ("CANA_BKP"), 4-byte version, 1-byte KDF id (0x01=Argon2id, 0x02=scrypt), KDF params (length-prefixed), 16-byte salt, 12-byte AES-GCM nonce (for AES-256-GCM) or 24-byte XChaCha20-Poly1305 nonce, 8-byte payload length; the 16-byte AES-GCM auth tag MUST follow the ciphertext as a trailer; for XChaCha20-Poly1305 likewise place its 16-byte tag after ciphertext; libsodium secretstream manages tags internally so no external trailer is required.
   3. WHEN encrypting payload THEN the system SHALL generate cryptographically-secure random nonces using randombytes_buf: AES-256-GCM uses 12-byte nonces, XChaCha20-Poly1305 uses 24-byte nonces; store the generated nonce with ciphertext metadata for reuse during decryption; for libsodium secretstream encryption, nonces are managed internally and no external nonce generation is needed; derive 256-bit key via the specified KDF and write [header‖ciphertext] to `.cbk`. No outer ZIP container after encryption.
