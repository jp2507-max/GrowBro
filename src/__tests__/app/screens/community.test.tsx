...
import CommunityScreen from '@/app/(app)/(index,community,strains,calendar)/community';
...
jest.mock('@/api', () => ({
  usePostsInfinite: (...args: unknown[]) => postsInfiniteMock(...args),
}));
...
type PostItem = { id?: string };

const createPage = (items: PostItem[] = []) => ({ results: items });

type HookState = ReturnType<typeof mockCommunityState>;

function mockCommunityState(): {
  data: { pages: { results: PostItem[] }[] } | undefined;
  isLoading: boolean;
  isError: boolean;
  error: Error | null;
