...
// Storage Manager for originals (documentDirectory) and derived cache (cacheDirectory)
// - Originals: never deleted automatically
// - Cache: LRU-evicted to a configurable size cap
...
const DEFAULT_CACHE_LIMIT_BYTES = 400 * 1024 * 1024; // 400 MB
...
// Split implementations to satisfy lint max-lines-per-function
...
async function storeImageImpl(
  ...
  let dest = getOriginalPath(id);
  try {
    const sha = await computeSha256OfFile(uri);
    dest = joinUri(originalDir, `${sha}`);
  } catch {
    dest = getOriginalPath(id);
  }
