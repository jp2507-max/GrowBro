...
BEGIN
  ...
  , ph_created AS (
    SELECT jsonb_agg(jsonb_build_object(
      ...
      'measured_at', floor(extract(epoch FROM measured_at) * 1000),
      'ph', ph,
      'ec_raw', ec_raw,
      'ec_25c', ec_25c,
      'temp_c', temp_c,
...
BEGIN
  ...
  SELECT
    ...
    to_timestamp(COALESCE((rec->>'measured_at')::bigint, 0) / 1000.0),
  ...
  FROM (
    SELECT (rec->>'id')::uuid AS id,
           ...
           to_timestamp(COALESCE((rec->>'measured_at')::bigint, 0) / 1000.0) AS measured_at,
