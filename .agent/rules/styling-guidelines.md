...
- **Palette first**: [src/components/ui/colors.js](cci:7://file:///c:/Users/Peter/GrowBro/src/components/ui/colors.js:0:0-0:0) → Tailwind classes & runtime tokens consume same palette.
- **Runtime theming**: [src/lib/nativewind-theme-provider.tsx](cci:7://file:///c:/Users/Peter/GrowBro/src/lib/nativewind-theme-provider.tsx:0:0-0:0) uses NativeWind `vars()` to inject CSS variables. Edit themes there to change colors.
- **Semantic classes**: Use `bg-background`, `bg-card`, `text-text-primary`, `text-text-secondary` for auto light/dark switching.
- **Legacy roles**: [src/lib/theme-tokens.ts](cci:7://file:///c:/Users/Peter/GrowBro/src/lib/theme-tokens.ts:0:0-0:0) defines roles for React Navigation, FlashMessage, focus rings.
- **Tailwind vs runtime**: Tailwind for static layout; runtime tokens for `style` props & native APIs.
