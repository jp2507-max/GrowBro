...
### Phase 1 — Establish “Lean Routes” structure (touch only what we must)

- Create `src/screens/` (kebab-case files) and move screen logic out of the first batch of routes we’ll be editing anyway:
- [`src/app/(app)/calendar.tsx`](<src/app/(app)/calendar.tsx>)
- [`src/app/settings/active-sessions.tsx`](src/app/settings/active-sessions.tsx)
- [`src/app/(app)/assessment/result.tsx`](<src/app/(app)/assessment/result.tsx>)
- [`src/app/settings/security.tsx`](src/app/settings/security.tsx)
- [`src/app/plants/[id].tsx`](src/app/plants/[id].tsx)
- Convert those route files into wrappers only, e.g.:
- `export { default } from '@/screens/...';`
- Keep _routing-only_ config (`Stack.Screen options`) either in the wrapper or the relevant layout file.
